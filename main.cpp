...
void readFile(vector <Employee>& v_emp){
    ...
    while(!fin.eof()){
        ...
        Employee temp;
        temp.empId = atoi(strEmpId.c_str());//converting to legacy c string c_str() since strEmpID is string and empID is int atoi means ascii to int
        temp.name = strName;
...
void printEmployees(vector <Employee> v_emp){
    ...
    cout << setw(EMP_ID_WIDTH) << left << "EmpId"
         ...
         << setw(EMP_NUMERIC_WIDTH) << right << "Basic (Rs)"
         << setw(EMP_NUMERIC_WIDTH) <<  right << "PF (Rs)"
         << setw(EMP_NUMERIC_WIDTH) << right << "HltIns(Rs)"
         << setw(EMP_NUMERIC_WIDTH) << right << "Net (Rs)"
         << endl;
    ...
    cout << setw(EMP_ID_WIDTH) << left << "Total"
         << setw(EMP_NAME_WIDTH) << left << "In (Rs)"
...
int main()
{
	cout<<"\t\t\t\tWelcome to Employee Maganement System!"<<endl;
	cout<<"If any changes are made or new employee is added or deleted make sure to save changes"<<endl;
